...
- id: pbk03fwqgoyi
  ...
  type: service
  hostname: workshop-host
  path: ?folder=/workspace/temporal-failure-proof-python/exercises/module02
  port: 8443
- id: yx9sls10q4nb
  title: Temporal UI
  type: service
...
in [button label="Terminal 2" background="#444CE7"](tab-1), execute a Workflow to do the same process with the failure-prone "real world" API:
```bash,run
